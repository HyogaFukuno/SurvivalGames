...
class ChestService {
    ...
    val tier1Chests = ConcurrentHashMap<Triple<Int, Int, Int>, Boolean>()
    val tier2Chests = ConcurrentHashMap<Triple<Int, Int, Int>, Boolean>()

    fun getChest(x: Int, y: Int, z: Int, world: World): Chest? {
        val location = Location(world, x.toDouble(), y.toDouble(), z.toDouble())
        return location.block.state as? Chest
    }

    fun castChest(x: Int, y: Int, z: Int, world: World): Chest? {
        val location = Location(world, x.toDouble(), y.toDouble(), z.toDouble())
    ...
    fun fillTier1Chest(chest: Chest?) { chest?.let { fill(it, tier1Materials) } }

    fun fillTier2Chest(chest: Chest?) { chest?.let { fill(it, tier2Materials) } }
