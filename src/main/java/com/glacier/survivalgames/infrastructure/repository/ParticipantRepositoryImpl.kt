...
import com.glacier.survivalgames.infrastructure.database.DatabaseManager
    ...
    override fun save(participant: Participant): Participant {
        ...
        return if (exist == null) {
            ...
            databaseManager.executeUpdate(
                sql = """
                    INSERT INTO participants (unique_id, points, wins, played, kills, chests, lifespan) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """.trimIndent(),
                params = listOf(
            ...
            databaseManager.executeUpdate(
                sql = """
                    UPDATE participants
                    SET points = ?, wins = ?, played = ?, kills = ?, chests = ?, lifespan = ?
                    WHERE unique_id = ?
                """.trimIndent(),
                params = listOf(
                    participant.points,
