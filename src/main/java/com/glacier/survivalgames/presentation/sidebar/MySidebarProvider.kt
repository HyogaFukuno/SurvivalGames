...
@InjectableComponent
class MySidebarProvider(val stateMachine: StateMachine<GameState>,
                        val participantService: GameParticipantService) : SidebarProvider {
    ...
    override fun getLines(mcPlayer: MCPlayer): List<SidebarLine> {
        ...
        val playing = participantService.players().size
        val watching = participantService.spectators().size
